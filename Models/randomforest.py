...
class RandomForestModel:
    ...
    def train(self, X_train, y_train, scaler=None):
        self.scaler = scaler
        print(f"[RF] 학습 중...  train={len(X_train)}")
    ...
    def evaluate(self, X_test, y_test):
        ...
        plt.close()
        print(f"[RF] 결과 저장: {RESULT_DIR}/eval.png")
    ...
    def cross_validate(self, X, y, cv=5):
        ...
        print(f"[RF] CV R²: {np.round(scores, 3)}  mean={scores.mean():.3f} ± {scores.std():.3f}")
        return scores
