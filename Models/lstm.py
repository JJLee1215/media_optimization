...
Network:
  LSTM(input=28, hidden=32) → last hidden state → Linear(1)
  Same structure as RNN but with cell state for long-term memory
...
class LSTMModel:
    ...
    def train(self, loaders, y_scaler):
        ...
        self.net      = _LSTMNet(input_size, config.RNN_HIDDEN_SIZE, config.RNN_NUM_LAYERS).to(self.device)
        ...
        print(f"[LSTM] Training...  params={sum(p.numel() for p in self.net.parameters()):,}")
        t0 = time.time()
        ...
        for epoch in range(1, config.RNN_EPOCHS + 1):
            ...
            if vl < best_val:
                ...
                torch.save(self.net.state_dict(), SAVE_PATH.parent / "lstm_tmp.pt")

            if epoch % 20 == 0 or epoch == 1:
                print(f"  epoch {epoch:>3} | train {tl:.4f} | val {vl:.4f} | best@{best_epoch}")

        self.net.load_state_dict(torch.load(SAVE_PATH.parent / "lstm_tmp.pt", weights_only=True))
        print(f"[LSTM] Training complete.  {time.time()-t0:.1f}s  best val: {best_val:.4f}@{best_epoch}")
    ...
    def evaluate(self, X_test, y_test):
        ...
        axes[0].set_title(f"LSTM  (R²={r2:.3f}  RMSE={rmse:.3f})")
        ...
        plt.close()
        print(f"[LSTM] Plot saved: {RESULT_DIR}/eval.png")
    ...
    def get_config(self):
        ...
        ※ hidden_size/num_layers/lr/epoch는 RNN과 동일한 config.RNN_* 값을
          공유해서 사용 중 (구조만 LSTM). best_epoch/best_val은 train()이
          끝난 뒤에만 알 수 있는 값이라 getattr로 방어적으로 조회함.
        """
        return {
            "epoch"       : config.RNN_EPOCHS,
    ...
    def load(self):
        ...
        self.net = _LSTMNet(ckpt["input_size"], config.RNN_HIDDEN_SIZE, config.RNN_NUM_LAYERS).to(self.device)
        self.net.load_state_dict(ckpt["state_dict"])
        self.y_scaler = ckpt["y_scaler"]
        print(f"[LSTM] Loaded: {SAVE_PATH}")
