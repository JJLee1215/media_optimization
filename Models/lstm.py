...
class LSTMModel:
    ...
    def train(self, loaders, y_scaler):
        ...
        self.net   = _ThinLSTM(input_size, cfg.RNN_HIDDEN_SIZE, cfg.RNN_NUM_LAYERS).to(self.device)
        ...
        print(f"[LSTM] 학습 시작  파라미터: {sum(p.numel() for p in self.net.parameters()):,}")
        t0 = time.time()
        ...
        print(f"[LSTM] 완료  {time.time()-t0:.1f}초  best val: {best_val:.4f}@{best_epoch}")
    ...
    def save(self):
        ...
        torch.save({"state_dict": self.net.state_dict(), "y_scaler": self.y_scaler,
                    "input_size": self.net.lstm.input_size}, SAVE_PATH)
        print(f"[LSTM] 저장: {SAVE_PATH}")

    def load(self):
        ckpt = torch.load(SAVE_PATH, map_location=self.device, weights_only=False)
        self.net = _ThinLSTM(ckpt["input_size"], cfg.RNN_HIDDEN_SIZE, cfg.RNN_NUM_LAYERS).to(self.device)
        self.net.load_state_dict(ckpt["state_dict"])
        self.y_scaler = ckpt["y_scaler"]
        print(f"[LSTM] 로드: {SAVE_PATH}")
