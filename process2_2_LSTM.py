"""
ts2_lstm.py
LSTM으로 시계열 → 최종 titer 예측

RNN과 구조 동일, nn.RNN → nn.LSTM으로만 교체
LSTM은 cell state를 추가로 가져 장기 의존성 더 잘 포착
"""
...
OUT_DIR   = "outputs/time_series/lstm"
...
def evaluate(model, X_test, y_test, y_scaler):
    ...
    axes[0].set_title(f"LSTM  (R²={r2:.3f})")
...
if __name__ == "__main__":
    ...
    model = ThinLSTM(input_size=len(feat_cols), hidden_size=32)
    print(f"[모델] LSTM  파라미터: {sum(p.numel() for p in model.parameters()):,}")
    ...
    with open(f"{OUT_DIR}/result.json", "w") as f:
        json.dump({"model":"LSTM","rmse":round(float(rmse),4),"r2":round(float(r2),4)}, f, indent=2)
    print(f"[완료] {OUT_DIR}/")